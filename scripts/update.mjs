...
function translateTransaction(description = '') {
  const role = s => s
    .replace(/\bGs\b/g, 'G').replace(/\bFs\b/g, 'F').replace(/\bCs\b/g, 'C')
    .replace(/\bG\s+(?=[A-Z])/g, 'G（ガード）').replace(/\bF\s+(?=[A-Z])/g, 'F（フォワード）')
    .replace(/\bC\s+(?=[A-Z])/g, 'C（センター）').replace(/\s+and\s+/gi, 'と');
  const translateSentence = raw => {
    ...
    return role(s)
      .replace(/three-team trade/gi, '3チーム間トレード').replace(/four-team trade/gi, '4チーム間トレード')
      .replace(/draft considerations/gi, 'ドラフト関連の権利').replace(/draft pick/gi, 'ドラフト指名権')
      .replace(/cash considerations/gi, '金銭').replace(/future considerations/gi, '将来の権利') + '。';
  };
  const protectedText = description.replaceAll('L.A.', 'L§A§').replaceAll('D.C.', 'D§C§');
  return protectedText.split(/(?<=\.)\s+/).filter(Boolean)
